...
type externalCredentialData struct {
	...
	PushSecrets *pushcrypto.PushKeys `json:"pushSecrets,omitempty"`
}
...
func NewExternalControlClientFromEnv() *ExternalControlClient {
	client := &ExternalControlClient{
		BaseURL:       strings.TrimRight(strings.TrimSpace(os.Getenv("ZERO_MESSENGER_CONTROL_URL")), "/"),
		Token:         strings.TrimSpace(os.Getenv("ZERO_MESSENGER_CONTROL_TOKEN")),
		AccountID:     strings.TrimSpace(os.Getenv("ZERO_MESSENGER_ACCOUNT_ID")),
		NativeLoginID: strings.TrimSpace(os.Getenv("ZERO_MESSENGER_NATIVE_LOGIN_ID")),
...
func hydrateCookies(platform types.Platform, input map[string]string) *cookies.Cookies {
	values := make(map[cookies.MetaCookieName]string, len(input))
	for key, value := range input {
		values[cookies.MetaCookieName(key)] = value
	}
	result := &cookies.Cookies{Platform: platform}
...
func (c *ExternalControlClient) StoreCredentials(
	ctx context.Context,
	loginID string,
	platform types.Platform,
	input *cookies.Cookies,
	loginUA string,
	pushSecrets *pushcrypto.PushKeys,
) (uint64, error) {
	if !c.OwnsLogin(loginID) {
	...
	request := externalCredentialWrite{
		...
		Credentials: externalCredentialData{
			Platform: platform, Cookies: cookieMap(input), LoginUA: loginUA, PushSecrets: pushSecrets,
		},
	}
	var response externalCredentialResponse
